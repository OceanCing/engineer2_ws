...
void Command::setVelocityFactorRadPerSecToInteger(rm_ecat::mit::CanBus bus, size_t id, double factor){
  size_t index = getIndex(bus, id);
...
void Command::setKpToInteger(rm_ecat::mit::CanBus bus, size_t id, double factor) {
  size_t index = getIndex(bus, id);
  kpToInteger_[index] = factor;
}

void Command::setKdToInteger(rm_ecat::mit::CanBus bus, size_t id, double factor) {
  size_t index = getIndex(bus, id);
  kdToInteger_[index] = factor;
}

void Command::setTorqueOffset(rm_ecat::mit::CanBus bus, size_t id, double factor) {
  size_t index = getIndex(bus, id);
  torqueOffset[index] = factor;
}

void Command::setVelocityOffset(rm_ecat::mit::CanBus bus, size_t id, double factor) {
  size_t index = getIndex(bus, id);
  velocityOffset[index] = factor;
}

void Command::setPositionOffset(rm_ecat::mit::CanBus bus, size_t id, double factor) {
  size_t index = getIndex(bus, id);
...
void Command::setTargetCommand(rm_ecat::mit::CanBus bus, size_t id, const rm_ecat::mit::target& target) {
  size_t index = getIndex(bus, id);
