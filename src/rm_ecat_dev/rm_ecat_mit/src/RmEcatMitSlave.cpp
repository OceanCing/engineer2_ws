...
#include "rm_ecat_mit/ObjectDictionary.h"
#include "rm_ecat_mit/RmEcatMitSlave.h"
#include "rm_ecat_mit/RxPdo.h"
#include "rm_ecat_mit/TxPdo.h"
...
RmEcatMitSlave::SharedPtr RmEcatMitSlave::deviceFromFile(const std::string& configFile, const std::string& name,
  ...
  if (!rm->loadConfigFile(configFile)) {
    MELO_ERROR_STREAM("[rm_ecat_slave::mit::RmEcatMitSlave::deviceFromFile] loading config file '" << configFile << "' for '" << name
                                                                                          << "' not successful.");
    throw std::runtime_error("[rm_ecat_slave::mit::RmEcatMitSlave::deviceFromFile] config file loading error");
  }
  return rm;
}
...
bool RmEcatMitSlave::startup() {
  ...
  if (!success) {
    MELO_ERROR_STREAM("[rm_ecat_slave::mit::RmEcatMitSlave::startup] hardware configuration of '" << name_ << "' not successful!");
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  return success;
}
...
void RmEcatMitSlave::setControlWord(rm_ecat::mit::Controlword controlword) {
