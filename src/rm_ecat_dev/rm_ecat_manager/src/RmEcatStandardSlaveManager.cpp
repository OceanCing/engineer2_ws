...
std::vector<bool> RmEcatStandardSlaveManager::getDigitalInputs() const {
  ...
  auto readings = getReadings<rm_ecat::standard::Reading>();
...
std::vector<rm_msgs::DbusData> RmEcatStandardSlaveManager::getDbusData() const {
  std::vector<rm_msgs::DbusData> dbusDatas_;
  for (const auto& slave : slaves_) {
