...
ClearSlaveManagerRosBase<SlaveManagerType>::ClearSlaveManagerRosBase(bool standalone, bool installSignalHandler, double timeStep,
                                                                     ros::NodeHandle& nh, std::string& rosPrefix)
    : SlaveManagerType(standalone, installSignalHandler, timeStep), nh_(nh), rosPrefix_(rosPrefix), shutdownPublishWorkerRequested_(false) {
  if (standalone) {
