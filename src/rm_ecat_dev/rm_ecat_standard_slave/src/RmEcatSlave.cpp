...
#include "rm_ecat_standard_slave/RmEcatSlave.h"
#include "rm_ecat_standard_slave/ConfigurationParser.h"
#include "rm_ecat_standard_slave/ObjectDictionary.h"
#include "rm_ecat_standard_slave/RxPdo.h"
#include "rm_ecat_standard_slave/TxPdo.h"
...
RmEcatStandardSlave::SharedPtr RmEcatStandardSlave::deviceFromFile(const std::string& configFile, const std::string& name,
  ...
  if (!rm->loadConfigFile(configFile)) {
    MELO_ERROR_STREAM("[rm_ecat_slave:RmEcatSlave::deviceFromFile] loading config file '" << configFile << "' for '" << name
                                                                                          << "' not successful.");
    throw std::runtime_error("[rm_ecat_slave:RmEcatSlave::deviceFromFile] config file loading error");
  }
  return rm;
}
...
bool RmEcatStandardSlave::startup() {
  ...
  if (!success) {
    MELO_ERROR_STREAM("[rm_ecat_slave:RmEcatSlave::startup] hardware configuration of '" << name_ << "' not successful!");
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  return success;
}
