...
#include "rm_ecat_standard_slave/ConfigurationParser.h"
...
bool getValueFromFile(YAML::Node& yamlNode, const std::string& varName, T& var) {
  if (!yamlNode[varName].IsDefined()) {
    MELO_WARN_STREAM("[rm_ecat::ConfigurationParser::getValueFromFile]: field '" << varName << "' is missing. Default value will be used.");
    return false;
  }
  try {
    ...
    MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::getValueFromFile] Error while parsing value \"" << varName
...
ConfigurationParser::ConfigurationParser(const std::string& filename) {
  ...
  try {
    ...
    MELO_FATAL_STREAM("[rm_ecat::ConfigurationParser::ConfigurationParser] Loading YAML configuration file '" << filename << "' failed.");
  }
  ...
  if (configNode["can_motors"].IsDefined()) {
    ...
    for (YAML::const_iterator it = motors.begin(); it != motors.end(); ++it) {
      ...
      if (bus == 0) {
        ...
        MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::parseConfiguration] Unknown can bus");
      }
    }
  }
    ...
    for (YAML::const_iterator it = imus.begin(); it != imus.end(); ++it) {
      ...
      if (child["bias"].IsDefined()) {
        ...
        if (bias.IsSequence() && bias.size() == 3) {
          ...
          MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::parseConfiguration] Bias must be a sequence of 3 elements");
        }
      }
      if (bus == 0) {
...
uint16_t ConfigurationParser::getMaxOut(MotorType motorType) {
  switch (motorType) {
    ...
    case MotorType::GM6020:
      return 30000;
    default:
      MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::getMaxOut] Unknown motor type");
      return 0;
  }
}
...
double ConfigurationParser::getTorqueFactorIntegerToNm(MotorType motorType) {
  switch (motorType) {
    ...
    default:
      MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::getTorqueFactorIntegerToNm] Unknown motor type");
      return 0;
  }
}
...
MotorType ConfigurationParser::getMotoTypeFromString(const std::string& motorTypeString) {
  if (motorTypeString == "M3508") {
    ...
    MELO_ERROR_STREAM("[rm_ecat::ConfigurationParser::getMotoTypeFromString] Unknown motor type");
    return MotorType::M3508;
  }
}
}  // namespace standard
