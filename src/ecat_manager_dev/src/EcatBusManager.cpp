...
#ifdef _MIT_FOUND_
            auto mit = rm_ecat::mit::RmEcatMitSlave::deviceFromFile(configurationFilePath, entry.name, entry.ethercatAddress);
            mit->setTimeStep(timeStep_);
            slave = mit;
#else
            throw std::runtime_error("rm_ecat_mit_slave not availabe.");
#endif
            break;
        }
        case EthercatSlaveType::Rm: {
#ifdef _RM_FOUND_
            auto rm = rm_ecat::standard::RmEcatStandardSlave::deviceFromFile(configurationFilePath, entry.name, entry.ethercatAddress);
            rm->setTimeStep(timeStep_);
            slave = rm;
#else
            throw std::runtime_error("rm_ecat_standard_slave not availabe.");
#endif
            break;
        }
        default:
